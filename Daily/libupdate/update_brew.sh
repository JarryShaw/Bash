...
case $arg_pkg in
    "all")
        for name in $arg_opkg ; do
            eval $logprefix echo -e "+ brew upgrade $name --cleanup $verbose $quiet" $seperator $logsuffix
            eval $logprefix brew upgrade $name --cleanup $verbose $quiet $seperator $logsuffix
            eval $logprefix echo $seperator $logsuffix
        done ;;
    *)
        flag=`brew list -1 | awk "/^$arg_pkg$/"`
        if [[ -nz $flag ]] ; then
            eval $logprefix echo -e "+ brew upgrade $arg_pkg --cleanup $verbose $quiet" $seperator $logsuffix
            eval $logprefix brew upgrade $arg_pkg --cleanup $verbose $quiet $seperator $logsuffix
            eval $logprefix echo $seperator $logsuffix
        else
            eval $logprefix echo -e "${blush}No package names $arg_pkg installed.${reset}" $seperator $logsuffix
